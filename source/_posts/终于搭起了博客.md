...
首先安装 `hexo` 和 `deployer`，`deployer` 用于将 `hexo` 部署到 `git page` 
...
<img src="https://raw.githubusercontent.com/CalmCenter/picGo/master/picturestest_ssh.png" style="zoom:50%">
...
<img src="https://raw.githubusercontent.com/CalmCenter/picGo/master/picturesgithub_list.png" style="zoom:50%">

2. 点击 clone 点击 Use SSH 并复制 `SSH` 路径

   <img src="https://raw.githubusercontent.com/CalmCenter/picGo/master/picturesgithub_ssh_path.png" style="zoom:50%">

3. 打开 `hexo` 目录，找到 `_config` 文件

   <img src="https://raw.githubusercontent.com/CalmCenter/picGo/master/pictureshexo_catalog_config.png" style="zoom:50%">
...
2.  clone 远程代码，复制刚才的 `SSH` 路径 或者 `HTTPS` 路径
...
<img src="https://raw.githubusercontent.com/CalmCenter/picGo/master/picturesgit_branch1.png" style="zoom:50%">

这里会告诉你本地当前所处的分支，切换分支可以用，**当你切换本地没有的远程分支是，它会自动给你拉取远程分支**

```
git checkout test 
```

7. 上图你可以看到本地分支比远程分支多一个 `test` 下一步就是将新分支推送至远程GitHub
...
8. 然后将 `.git` 以外的所有文件删掉，将 `hexo` 源码复制到  `D:\blog\github_hexo` 下，查看
